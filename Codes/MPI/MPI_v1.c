...
int main(int argc, char **argv)
{
	...
	if (my_rank == 0)
	{
		...
		MPI_Send(to_send, 3, MPI_INT, 1, 10, MPI_COMM_WORLD);
		MPI_Send(instance->options, instance->n_options, MPI_INT, 1, 10, MPI_COMM_WORLD);
		MPI_Send(instance->ptr, instance->n_items, MPI_INT, 1, 9, MPI_COMM_WORLD);
	...
	if (my_rank == 1)
	{
		...
		MPI_Recv(to_recv, 3, MPI_INT, 0, 10, MPI_COMM_WORLD, &status);

		printf("Reçu : %d / %d / %d\n",to_recv[0], to_recv[1], to_recv[2]);
		int *recv_opt = malloc(to_recv[2]*sizeof(int));
		int *recv_items = malloc(to_recv[1]*sizeof(int));

		MPI_Recv(recv_opt, to_recv[2], MPI_INT, 0, 10, MPI_COMM_WORLD, &status);
		MPI_Recv(recv_items, to_recv[1], MPI_INT, 0, 9, MPI_COMM_WORLD, &status);
